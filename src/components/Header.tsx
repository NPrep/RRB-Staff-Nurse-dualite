      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
        <div className="flex justify-between items-center h-20">
          ...
          <nav className="hidden md:flex space-x-1 items-center h-full">
            {navLinks.map((link) => (
              <Link key={link.path} to={link.path} className="px-4 py-2 rounded-md font-medium text-sm transition-colors no-underline text-slate-700 hover:bg-rrb-light hover:text-rrb-red">
                {link.name}
              </Link>
            ))}
          </nav>

          <button className="md:hidden p-2 text-slate-700 hover:text-rrb-red transition-colors z-50 relative" onClick={() => setIsMenuOpen(!isMenuOpen)}>
      ...
      {isMenuOpen && (
        <div className="md:hidden nprep-mobile-drawer">
          <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
            {navLinks.map((link) => (
              <Link key={link.path} to={link.path} className="block px-3 py-3 text-base font-medium rounded-md no-underline text-slate-700 hover:text-rrb-red hover:bg-rrb-light" onClick={() => setIsMenuOpen(false)}>
                {link.name}
              </Link>
            ))}
          </div>
